...
// Windows calling conventions.  On x86-64 there is exactly one (ms_abi); on
// i386 three are in play and picking the wrong one corrupts the stack:
//   stdcall  — callee pops args.  Nearly all of Win32.
//   cdecl    — caller pops args.  Mandatory for variadics (the callee cannot
//              know how many bytes to pop) and used by the msvcrt CRT.
//   thiscall — `this` in ECX, rest on the stack, callee pops.  COM vtables.
// None of these decorate symbol names on i386 ELF (@N decoration is a
// PE/COFF convention), so the kernel32_Foo naming scheme is unchanged.
#define WINAPI   __attribute__((stdcall))
#define CDECLAPI __attribute__((cdecl))
...
// CRITICAL_SECTION — opaque block with pthread_mutex_t overlaid.  A Win32
// CRITICAL_SECTION is 24 bytes and i386 pthread_mutex_t is also 24, so the
// natural size happens to be exactly big enough; keep 40 anyway to match
// what MSVC-era headers reserve and leave slack.
struct CRITICAL_SECTION { uint8_t opaque[40]; };

// WIN32_FIND_DATAA — no pointer members, so still 320 bytes on i386
