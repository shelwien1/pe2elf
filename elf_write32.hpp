...
// ---------------------------------------------------------------------------
// Writer: serializes ELF32 content from Builder+Plan+PeImage to a file
// ---------------------------------------------------------------------------
struct Writer {
  const PeImage& image;
  const Plan& plan;
  ...
  bool write(const char* out_path, uint64_t shoff, uint64_t shstrtab_foff) {
    ...
    ehdr.e_entry = (uint32_t)(build.so_mode ? build.safe_entry_va : plan.trampoline_va);
    ehdr.e_phoff = sizeof(Elf32_Ehdr);
    ehdr.e_shoff = keep_shdr ? (uint32_t)shoff : 0;
    ehdr.e_flags = 0;
    ...
    out.append(build.dynsym_data.data(), build.dynsym_data.size()*sizeof(Elf32_Sym));
    ...
    out.pad_to_size(plan.rel_foff);
    out.append(build.rel_data.data(), build.rel_data.size()*sizeof(Elf32_Rel));
    out.pad_to_size(plan.dynamic_foff);
    out.append(build.dynamic_data.data(), build.dynamic_data.size()*sizeof(Elf32_Dyn));
