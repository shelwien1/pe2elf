#pragma once
// Thread-related kernel32 surface — included from shim32.cpp AFTER
// shim32_kernel32_sync.hpp so it can rely on the sync types & helpers
...
// Win32 affinity masks are ULONG_PTR, i.e. 32-bit here — writing 8 bytes into
// the caller's 4-byte slot would smash the word after it.
extern "C" EXPORT BOOL kernel32_GetProcessAffinityMask(HANDLE /*h*/, ULONG_PTR* proc_mask, ULONG_PTR* sys_mask) {
  cpu_set_t cs; CPU_ZERO(&cs);
  sched_getaffinity(0, sizeof(cs), &cs);
  ULONG_PTR mask = 0;
  for( int i = 0; i < (int)(sizeof(ULONG_PTR)*8); ++i )
    if( CPU_ISSET(i, &cs) ) mask |= ((ULONG_PTR)1 << i);
  if( proc_mask ) *proc_mask = mask;
  if( sys_mask  ) *sys_mask  = mask;
  return TRUE;
}
extern "C" EXPORT BOOL kernel32_SetProcessAffinityMask(HANDLE /*h*/, ULONG_PTR /*mask*/) { return TRUE; }
