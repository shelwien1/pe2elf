#pragma once
// user32 surface — included from shim32.cpp.
//
...
//
// References from shim32.cpp: utf8_to_wchar, wchar_to_utf8, g_image_base,
// g_pe_base.
...
static int load_string_impl(const void* hmod, UINT id, char* bufA, uint16_t* bufW, int bufmax) {
  ...
  // PE32 (0x10B): data directory starts at opt+96.  (PE32+ is 0x20B with the
  // directory at opt+112 — that is the x64 tool's case, not ours.)
  if( *(uint16_t*)opt != 0x010B ) return 0;
  uint32_t rsrc_rva  = *(uint32_t*)(opt + 96 + 16);
  uint32_t rsrc_size = *(uint32_t*)(opt + 96 + 20);
