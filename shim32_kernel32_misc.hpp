...
// from shim32.cpp before shim32_kernel32_sync.hpp.
//
...
// LocalFree.
//
// References from shim32.cpp: g_image_base, FAKE_WIN_MODULE,
...
#endif
// MEMORY_BASIC_INFORMATION, Win32 layout (28 bytes — the Win64 form is 48,
// with 8-byte pointer fields and RegionSize at +24):
//  +0  PVOID     BaseAddress
//  +4  PVOID     AllocationBase
//  +8  DWORD     AllocationProtect
//  +12 SIZE_T    RegionSize
//  +16 DWORD     State
//  +20 DWORD     Protect
//  +24 DWORD     Type
#define WIN32_MBI_SIZE 28
extern "C" EXPORT size_t kernel32_VirtualQuery(LPCVOID addr, void* buf, size_t buflen) {
  if( !buf || buflen < WIN32_MBI_SIZE ) { SET_LAST_ERROR(ERROR_INVALID_PARAMETER); return 0; }
  uint8_t* mbi = (uint8_t*)buf;
  memset(mbi, 0, WIN32_MBI_SIZE);
  ...
  if( f ) {
    ...
    while( fgets(line, sizeof(line), f) ) {
      ...
      if( s <= target && target < e ) {
        ...
        *(uint32_t*)(mbi+0)  = (uint32_t)s;
        *(uint32_t*)(mbi+4)  = (uint32_t)s;
        *(uint32_t*)(mbi+8)  = prot;
        *(uint32_t*)(mbi+12) = (uint32_t)(e - s);
        *(uint32_t*)(mbi+16) = MEM_COMMIT;
        *(uint32_t*)(mbi+20) = prot;
        *(uint32_t*)(mbi+24) = MEM_PRIVATE;
        fclose(f); return WIN32_MBI_SIZE;
      }
    }
    fclose(f);
  }
  *(uint32_t*)(mbi+0)  = (uint32_t)(target & ~(uintptr_t)0xFFF);
  *(uint32_t*)(mbi+12) = 0x1000;
  *(uint32_t*)(mbi+16) = MEM_FREE;
  return WIN32_MBI_SIZE;
}
