#pragma once
// FLS (Fiber Local Storage) — included from shim32.cpp.
//
...
// array; forward-declared in shim32.cpp and registered as g_tls_slots_key's
// destructor up there, but defined here so it can see g_fls_callbacks.
//
// References from shim32.cpp: g_tls_alloc_mu, g_tls_alloc_used,
// g_tls_static_idx, tls_get_slots.

#define FLS_MAX_SLOTS 64
// g_tls_alloc_mu and g_tls_alloc_used are shared with TLS (declared in shim32.cpp).
...
typedef void (__attribute__((stdcall)) *fls_callback_t)(void*);

// Single pthread_key_create destructor for the per-thread slots array
// (registered as g_tls_slots_key's destructor up at the top of shim32.cpp).
