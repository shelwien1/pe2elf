#pragma once
// Tail of the kernel32 surface — included from shim32.cpp.
//
...
// DeviceIoControl.
//
// References from shim32.cpp: utf8_to_wchar, wchar_to_utf8,
...
// and dispatch from a background thread.  An explicit stdcall trampoline
// makes the cdecl (pthread) → stdcall (PHANDLER_ROUTINE) ABI transition
// obvious to readers and to the compiler.
// ---------------------------------------------------------------------------
typedef BOOL (__attribute__((stdcall)) *PHANDLER_ROUTINE)(DWORD);
...
// (cdecl); the indirect call through `fn` switches to stdcall for the
